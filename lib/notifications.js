...
import { sendPushForNotification } from './pushSend';
...
export async function createNotification({ userEmail, type, taskId, taskName, dueDate, dedupeKey, title, body }) {
  ...
  if (!email || !dedupeKey) {
    console.log(`[push] create-skipped ${JSON.stringify({ created: false, reason: 'missing-target', type: String(type || ''), taskId: String(taskId || ''), email })}`);
    return { created: false, reason: 'missing-target' };
  }
  const existing = await adminDb.collection('notifications').where('dedupeKey', '==', dedupeKey).limit(1).get();
  if (!existing.empty) {
    console.log(`[push] create-skipped ${JSON.stringify({ created: false, reason: 'duplicate', type: String(type || ''), taskId: String(taskId || ''), email, notificationId: existing.docs[0].id })}`);
  ...
  console.log(`[push] created ${JSON.stringify({ created: true, notificationId: ref.id, type: String(type || ''), taskId: String(taskId || ''), email })}`);
  try {
    ...
    console.log(`[push] after-send ${JSON.stringify({ notificationId: ref.id, type: String(type || ''), taskId: String(taskId || ''), email, ...pushResult })}`);
  } catch (err) {
    console.error('Push delivery failed', err);
    console.log(`[push] after-send ${JSON.stringify({ notificationId: ref.id, type: String(type || ''), taskId: String(taskId || ''), email, skipped: 'threw', error: String((err && err.message) || err) })}`);
  }
...
export async function notifyCancelRequested({ taskId, creatorEmail, taskName, dueDate, reason }) {
  const email = String(creatorEmail || '').toLowerCase().trim();
  if (!email) return { created: false };
  return createNotification({
    userEmail: email,
    ...
    taskName,
    dueDate,
    dedupeKey: `cancelRequested:${taskId}:${Date.now()}`,
    title: 'Cancel Request',
    body: `Cancel requested for ${taskName || 'a task'}: ${reason || 'No reason given.'}`,
  });
}

export async function notifyCancelApproved({ taskId, assigneeEmail, taskName, dueDate }) {
  const email = String(assigneeEmail || '').toLowerCase().trim();
  if (!email) return { created: false };
  return createNotification({
    userEmail: email,
    type: 'cancelApproved',
    ...
    taskName,
    dueDate,
    dedupeKey: `cancelApproved:${taskId}:${Date.now()}`,
    title: 'Task Cancelled',
    body: `${taskName || 'A task'} was cancelled.`,
  });
}

export async function notifyCancelDenied({ taskId, assigneeEmail, taskName, dueDate }) {
  const email = String(assigneeEmail || '').toLowerCase().trim();
  if (!email) return { created: false };
  return createNotification({
    userEmail: email,
    ...
    taskName,
    dueDate,
    dedupeKey: `cancelDenied:${taskId}:${Date.now()}`,
    title: 'Cancel Request Denied',
    body: `The cancel request for ${taskName || 'a task'} was denied.`,
  });
}
...
export async function notifyOverdue({ taskId, assignedTo, taskName, dueDate }) {
  const email = String(assignedTo || '').toLowerCase().trim();
  if (!email || !dueDate) return { created: false };
  return createNotification({
    userEmail: email,
    ...
    taskName,
    dueDate,
    dedupeKey: `overdue:${taskId}:${email}:${dueDate}`,
    title: 'Task Overdue',
    body: `${taskName || 'A task'} is overdue. It was due ${formatDeadline(dueDate)}.`,
  });
}
...
export async function runDueNotificationSweep() {
  ...
  const results = { dueToday: 0, dueSoon: 0, overdue: 0, skipped: 0 };
  const skip = { Done: true, Cancelled: true, 'Cancel Requested': true, 'On Hold': true, Pending: true };
  for (const doc of snap.docs) {
    const t = doc.data() || {};
    if (skip[t.status]) { results.skipped += 1; continue; }
    ...
    if (t.deadline === today) {
      ...
      const r = await notifyOverdue({ taskId: doc.id, assignedTo: t.assignedTo, taskName: t.task, dueDate: t.deadline });
      if (r.created) results.overdue += 1;
