...
export const MODELS = ['Revenue Sharing', 'Corporate Wellness', 'Mall Paid Space'];
...
function mallFromGross(G, mallPct, processingPct) {
  const mallShare = G * n(mallPct) / 100;
  const processing = G * n(processingPct) / 100;
  return { gross: G, mallShare, processing, netBeforeOpex: G - mallShare - processing };
}
...
export function compute(a) {
  ...
  const buildScenario = (spd) => {
    ...
    const split = a.businessModel === 'Mall Paid Space'
      ? mallFromGross(G, a.mallRevSharePct, processingPct)
  ...
  if (M > 0) {
    ...
    const denom = a.businessModel === 'Mall Paid Space'
      ? 1 - n(a.mallRevSharePct) / 100 - processingPct / 100
