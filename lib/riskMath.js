...
export const RISK_LEVELS = ['Not Assessed', 'Low', 'Medium', 'High', 'Critical'];
export const RISK_STATUSES = ['Not Assessed', 'Open', 'Mitigating', 'Resolved', 'Not Applicable'];
...
export const CHINA_LOGISTICS = [
  ['chinaFreight', 'China-Side Freight / Export Handling'],
  ['intlFreight', 'International Freight (Ocean / Air)'],
...
export function emptyAssessment() {
  ...
  return {
    ...
    risks: RISK_CATEGORIES.map((category) => ({ category, level: 'Not Assessed', status: 'Not Assessed', owner: '', mitigation: '', notes: '' })),
    notes: '', outstandingInfo: '', decisionDate: '', approvedBy: '', archived: false, version: 1,
  };
}

export function normalizeLogistics(a) {
  const items = (a.logisticsItems || []).map((i) => ({ ...i }));
  const byKey = {};
  items.forEach((i) => { if (i.key) byKey[i.key] = i; });
  if (byKey.chinaFreight) byKey.chinaFreight.label = 'China-Side Freight / Export Handling';
  if (byKey.intlFreight) byKey.intlFreight.label = 'International Freight (Ocean / Air)';
  if (byKey.oceanAir) {
    if (!byKey.intlFreight) {
      byKey.intlFreight = { key: 'intlFreight', label: 'International Freight (Ocean / Air)', amount: byKey.oceanAir.amount || '', status: byKey.oceanAir.status || 'Assumption', source: byKey.oceanAir.source || '', date: byKey.oceanAir.date || '', notes: byKey.oceanAir.notes || '' };
    } else {
      const oceanAmt = Number(byKey.oceanAir.amount) || 0;
      const intlAmt = Number(byKey.intlFreight.amount) || 0;
      if (oceanAmt && !intlAmt) {
        byKey.intlFreight.amount = byKey.oceanAir.amount;
        if (byKey.oceanAir.status && (!byKey.intlFreight.status || byKey.intlFreight.status === 'Assumption')) {
          byKey.intlFreight.status = byKey.oceanAir.status;
        }
      } else if (oceanAmt && intlAmt) {
        byKey.intlFreight.amount = String(oceanAmt + intlAmt);
      }
      byKey.intlFreight.label = 'International Freight (Ocean / Air)';
    }
    delete byKey.oceanAir;
  }
  if (a.inventorySource === 'China Direct') {
    return {
      ...a,
      logisticsItems: CHINA_LOGISTICS.map(([k, l]) => byKey[k] || { key: k, label: l, amount: '', status: 'Assumption', source: '', date: '', notes: '' }),
    };
  }
  return { ...a, logisticsItems: items.filter((i) => i.key !== 'oceanAir').map((i) => byKey[i.key] || i) };
}
...
export function applySourceLines(a) {
  ...
  const next = { ...a, logisticsItems: keys.map(([k, l]) => prev[k] || { key: k, label: l, amount: '', status: 'Assumption', source: '', date: '', notes: '' }) };
  return normalizeLogistics(next);
}
