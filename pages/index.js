...
export default function Home() {
  ...
  const metrics = useMemo(() => {
    ...
    const chairs = Number(selected.numberOfChairs) > 0 ? Number(selected.numberOfChairs) : 2;
    const feePerChair = Number(selected.monthlyFee) || 0;
    ...
    if (selected.goLiveDate) {
      ...
      if (breakEvenDate) daysToBreakEven = `${Math.round((new Date(breakEvenDate) - goLive) / 86400000)} days`;
      else daysToBreakEven = `${Math.round((new Date() - goLive) / 86400000)} days (Still Recovering)`;
    }
    const byCategory = {};
    ...
    return { totalExpenses, totalLemoIncome, netProfitLoss, roiProgress, currentMonthlyRevenue, breakEvenDate: breakEvenDate || 'Not Reached', daysToBreakEven, expenseBreakdown, monthlyIncome };
  ...
  async function submitExpense(e) {
    ...
    setExpForm({ date: '', category: '', item: '', description: '', source: '', quantity: 1, costPerUnit: '', notes: '' }); setShowExpenseModal(false); loadDetail(selectedName);
  }
  ...
  async function downloadExpensePdf() {
    ...
    doc.setFontSize(9); doc.setTextColor(112, 107, 102);
    ...
    <Layout active={tab} onNavigate={navigate}>
      {!selected ? (
        <>
          ...
          {!loading && filteredProjects.length === 0 && (<p className="muted">No installations match this filter. {projects.length === 0 && <>An admin can bring in the old Project Details sheet from <a href="/admin/import">Import Data</a>.</>}</p>)}
          {!loading && filteredProjects.length > 0 && (
            <div className="table-wrap"><table>
              <thead><tr><th>Name</th><th>Business Model</th><th>State</th><th>Chairs</th><th>Fee / chair</th><th>Monthly contract</th></tr></thead>
              <tbody>{filteredProjects.map((p) => { const chairs = Number(p.numberOfChairs) > 0 ? Number(p.numberOfChairs) : 2; const fee = Number(p.monthlyFee); const contract = p.businessModel === 'Corporate Wellness' && !isNaN(fee) ? fee * chairs : null; return (<tr key={p.id} onClick={() => setSelectedName(p.name)} style={{ cursor: 'pointer' }}><td>{p.name}</td><td>{p.businessModel}</td><td>{p.state}</td><td>{p.numberOfChairs != null && p.numberOfChairs !== '' ? p.numberOfChairs : chairs}</td><td>{p.monthlyFee != null && p.monthlyFee !== '' ? `$${p.monthlyFee}` : ''}</td><td>{contract != null ? `$${contract}` : ''}</td></tr>); })}</tbody>
            </table></div>
          )}
        </>
      ) : (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>{isAdmin && <button className="btn" onClick={openEditAccount}>Edit Company</button>}</div>
          </div>
          <h2>{selected.name}</h2>
          <div className="card"><div className="grid-3">
            ...
            <div>
              <div className="muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase' }}>BD Consultant</div><div>{selected.bdConsultantName || '\u2014'}</div><div className="muted">{selected.bdConsultantPhone}</div><div className="muted">{selected.bdConsultantEmail}</div>
              <div className="muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase', marginTop: 12 }}>Chairs</div>
              <div>{selected.numberOfChairs != null && selected.numberOfChairs !== '' ? selected.numberOfChairs : 2}</div>
              {selected.businessModel === 'Corporate Wellness' && (<><div className="muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase', marginTop: 12 }}>Fee / chair</div><div>{selected.monthlyFee != null && selected.monthlyFee !== '' ? `$${selected.monthlyFee}` : '\u2014'}</div><div className="muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase', marginTop: 12 }}>Monthly contract</div><div>{fmt((Number(selected.monthlyFee) || 0) * (Number(selected.numberOfChairs) > 0 ? Number(selected.numberOfChairs) : 2))}</div></>)}
            </div>
          </div></div>
...
function Kpi({ label, value, negative }) {
  return (<div className="card" style={{ marginBottom: 0 }}><div className="muted" style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8 }}>{label}</div><div style={{ fontFamily: "'Lora', serif", fontSize: '1.5rem', color: negative ? 'var(--ember-muted)' : 'var(--obsidian)' }}>{value}</div></div>);
}
