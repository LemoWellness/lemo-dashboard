...
export default function Home() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      breakEvenDate: breakEvenDate || 'Not Reached',
      daysToBreakEven,
      expenseBreakdown,
  ...
  async function downloadExpensePdf() {
    ...
    autoTable(doc, {
      ...
      body: expenses.map((e) => [e.date || '', e.category || '', e.item || '', e.source || '', e.description || '', e.costPerUnit != null ? `$${Number(e.costPerUnit).toFixed(2)}` : '', e.quantity ?? '', e.notes || '']),
      headStyles: { fillColor: [12, 10, 9] }, styles: { fontSize: 8 },
    });
  ...
  return (
    <Layout active={tab} onNavigate={navigate}>
      {!selected ? (
        <>
          ...
          {!loading && filteredProjects.length === 0 && (
            <p className="muted">No installations match this filter. {projects.length === 0 && <>An admin can bring in the old Project Details sheet from <a href="/admin/import">Import Data</a>.</>}</p>
          )}
          {!loading && filteredProjects.length > 0 && (
      ...
      {showExpenseModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,10,9,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <form className="card" onSubmit={submitExpense} style={{ background: 'var(--warm-white)', width: '90%', maxWidth: 460, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <label style={{ display: 'block', marginBottom: 12 }}>Category<input value={expForm.category} onChange={(e) => setExpForm({ ...expForm, category: e.target.value })} required style={{ width: '100%', marginTop: 4 }} /></label>
            <label style={{ display: 'block', marginBottom: 12 }}>Item<input value={expForm.item} onChange={(e) => setExpForm({ ...expForm, item: e.target.value })} style={{ width: '100%', marginTop: 4 }} /></label>
            <div className="form-grid-2" style={{ marginBottom: 12 }}>
              <label>Quantity<input type="number" min="0" value={expForm.quantity} onChange={(e) => setExpForm({ ...expForm, quantity: e.target.value })} required /></label>
              <label>Cost / Unit<input type="number" min="0" step="0.01" value={expForm.costPerUnit} onChange={(e) => setExpForm({ ...expForm, costPerUnit: e.target.value })} required /></label>
            </div>
      ...
      {showIncomeModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,10,9,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <form className="card" onSubmit={submitIncome} style={{ background: 'var(--warm-white)', width: '90%', maxWidth: 420 }}>
            ...
            <label style={{ display: 'block', marginBottom: 16 }}>Amount<input type="number" min="0" step="0.01" value={incForm.amount} onChange={(e) => setIncForm({ ...incForm, amount: e.target.value })} required style={{ width: '100%', marginTop: 4 }} /></label>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button type="button" className="btn" onClick={() => setShowIncomeModal(false)}>Cancel</button>
      ...
      {showNoteModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,10,9,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <form className="card" onSubmit={submitNote} style={{ background: 'var(--warm-white)', width: '90%', maxWidth: 440 }}>
            <h3 style={{ marginTop: 0 }}>Communication Log</h3>
            <label style={{ display: 'block', marginBottom: 12 }}>Note<textarea value={noteForm.note} onChange={(e) => setNoteForm({ ...noteForm, note: e.target.value })} rows={4} required style={{ width: '100%', marginTop: 4 }} /></label>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button type="button" className="btn" onClick={() => setShowNoteModal(false)}>Cancel</button>
...
function Collapsible({ title, open, onToggle, actions, children }) {
  return (
    <div className="card">
      <div onClick={onToggle} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', flexWrap: 'wrap', gap: 10, marginBottom: open ? 14 : 0 }}>
