...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    if (!hit) return;
    setSubtab(hit.status === 'Done' ? 'completed' : (hit.status === 'On Hold' || hit.status === 'Pending') ? 'hold' : 'active');
    setOpenTask(hit);
    setEditing(false);
    setUpdateText('');
    setPendingStatus('');
  ...
  const activeCount = tasks.filter((t) => t.status !== 'Done' && !holdStatuses(t.status)).length;
  const holdCount = tasks.filter((t) => holdStatuses(t.status)).length;
  const completedCount = tasks.filter((t) => t.status === 'Done').length;

  const filtered = useMemo(() => {
    let list = tasks.filter((t) => {
      if (subtab === 'active' && (t.status === 'Done' || t.status === 'On Hold' || t.status === 'Pending')) return false;
      if (subtab === 'hold' && t.status !== 'On Hold' && t.status !== 'Pending') return false;
  ...
  async function changeStatus(id, status, note) {
    ...
    setPendingStatus('');
    setStatusNote('');
    if (status === 'On Hold' || status === 'Pending') setSubtab('hold');
    else if (status === 'Done') setSubtab('completed');
    else setSubtab('active');
    load();
  }
      ...
      {loading ? <p className="muted">Loading tasks...</p> : loadError ? <p className="form-error">{loadError}</p> : (
        <>
          <div className="seg-tabs">
            ...
            <button className={`seg-tab ${subtab === 'hold' ? 'active' : ''}`} onClick={() => setSubtab('hold')}>
              Pending / Hold ({holdCount})
            </button>
          ...
          <div className="card">
            ...
            <table>
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={6} className="muted">{subtab === 'completed' ? 'No completed tasks yet.' : subtab === 'hold' ? 'No pending or on-hold tasks.' : 'No active tasks - click + Add Task to create one.'}</td></tr>
                )}
              </tbody>
            </table>
            </div>
