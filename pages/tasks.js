...
export default function Tasks() {
  ...
  const holdStatuses = (s) => s === 'On Hold' || s === 'Pending';
  const parked = (s) => s === 'Done' || s === 'Cancelled' || holdStatuses(s);
  const activeCount = tasks.filter((t) => !parked(t.status)).length;
  const holdCount = tasks.filter((t) => holdStatuses(t.status)).length;
  const cancelledCount = tasks.filter((t) => t.status === 'Cancelled').length;
  const completedCount = tasks.filter((t) => t.status === 'Done').length;

  const filtered = useMemo(() => {
    let list = tasks.filter((t) => {
      if (subtab === 'active' && parked(t.status)) return false;
      if (subtab === 'hold' && !holdStatuses(t.status)) return false;
      if (subtab === 'cancelled' && t.status !== 'Cancelled') return false;
  ...
  function startEdit() {
    ...
    setEditForm({ assignedTo: openTask.assignedTo, task: openTask.task, deadline: openTask.deadline || '', priority: openTask.priority, notes: '' });
    setEditing(true);
    setFormError('');
  }
  ...
  function requestStatus(next) {
    ...
    if (next === 'On Hold' || next === 'Pending') { setPendingStatus(next); setStatusNote(''); setFormError(''); return; }
    setPendingStatus('');
    setStatusNote('');
  ...
  async function changeStatus(id, status, note) {
    ...
    if (!res.ok) { setFormError((await res.json()).error || 'Could not update status.'); return; }
    setPendingStatus('');
    setStatusNote('');
    setSubtab(tabFor(status));
    load();
  }
  ...
  async function submitCancelRequest(e) {
    e.preventDefault();
    if (!openTask || !cancelReason.trim()) { setFormError('Write why this task should be cancelled.'); return; }
    setSaving(true);
    const res = await authedFetch(`/api/tasks/${openTask.id}`, { method: 'PATCH', body: JSON.stringify({ cancelRequest: cancelReason.trim() }) });
    setSaving(false);
    if (!res.ok) { setFormError((await res.json()).error || 'Could not request cancel.'); return; }
    setShowCancel(false);
    setCancelReason('');
    setSubtab('active');
    load();
  ...
  async function decideCancel(decision) {
    if (!openTask) return;
    setSaving(true);
    const res = await authedFetch(`/api/tasks/${openTask.id}`, { method: 'PATCH', body: JSON.stringify({ cancelDecision: decision }) });
    setSaving(false);
    if (!res.ok) { setFormError((await res.json()).error || 'Could not update cancel request.'); return; }
    setSubtab(decision === 'approve' ? 'cancelled' : 'active');
    load();
  }
  ...
  const live = openTask;
  const emptyMsg = subtab === 'completed' ? 'No completed tasks yet.' : subtab === 'hold' ? 'No pending or on-hold tasks.' : subtab === 'cancelled' ? 'No cancelled tasks.' : 'No active tasks.';
      ...
      {loading ? <p className="muted">Loading tasks...</p> : loadError ? <p className="form-error">{loadError}</p> : (
        <>
          <div className="seg-tabs">
            <button className={`seg-tab ${subtab === 'active' ? 'active' : ''}`} onClick={() => setSubtab('active')}>Active ({activeCount})</button>
            <button className={`seg-tab ${subtab === 'hold' ? 'active' : ''}`} onClick={() => setSubtab('hold')}>Pending / Hold ({holdCount})</button>
            <button className={`seg-tab ${subtab === 'cancelled' ? 'active' : ''}`} onClick={() => setSubtab('cancelled')}>Cancelled ({cancelledCount})</button>
            <button className={`seg-tab ${subtab === 'completed' ? 'active' : ''}`} onClick={() => setSubtab('completed')}>Completed ({completedCount})</button>
          </div>
            ...
            {subtab === 'active' && (
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: '0.75rem' }} className="muted">Status
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={{ minWidth: 130 }}>
                  <option value="">All</option>
                  <option>Not Started</option><option>In Progress</option><option>Cancel Requested</option>
                </select>
              </label>
            )}
          ...
          <div className="card">
            <div className="table-wrap">
              <table>
                <thead><tr><th>Task</th><th>For</th><th>Added By</th><th>Deadline</th><th>Priority</th><th>Status</th></tr></thead>
                <tbody>
                  {filtered.map((t) => {
                  ...
                  {filtered.length === 0 && <tr><td colSpan={6} className="muted">{emptyMsg}</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,10,9,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <form className="card" onSubmit={submitAdd} style={{ background: 'var(--warm-white)', width: '90%', maxWidth: 440, maxHeight: '88vh', overflowY: 'auto' }}>
            ...
            <label style={{ display: 'block', marginBottom: 12 }}>Task
              <textarea value={form.task} onChange={(e) => setForm({ ...form, task: e.target.value })} rows={3} style={{ width: '100%', boxSizing: 'border-box', padding: 8, border: '1px solid var(--iron)', borderRadius: 4, marginTop: 4, fontFamily: 'inherit' }} />
            </label>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,10,9,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => { setOpenTask(null); setEditing(false); }}>
          <div className="card" style={{ background: 'var(--warm-white)', width: '92%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
              <h3 style={{ marginTop: 0 }}>{editing ? 'Edit Task' : live.task}</h3>
            ...
            {editing ? (
              <form onSubmit={submitEdit}>
                <label style={{ display: 'block', marginBottom: 12 }}>Assigned To<select value={editForm.assignedTo} onChange={(e) => setEditForm({ ...editForm, assignedTo: e.target.value })} style={{ width: '100%', marginTop: 4 }}>{sortedUsers.map((u) => <option key={u.email} value={u.email}>{u.name}</option>)}</select></label>
                <label style={{ display: 'block', marginBottom: 12 }}>Task<textarea value={editForm.task} onChange={(e) => setEditForm({ ...editForm, task: e.target.value })} rows={3} style={{ width: '100%', boxSizing: 'border-box', padding: 8, border: '1px solid var(--iron)', borderRadius: 4, marginTop: 4, fontFamily: 'inherit' }} /></label>
                <div className="form-grid-2" style={{ marginBottom: 12 }}>
            ...
            {live.status === 'Cancelled' && live.cancelReason && <p style={{ marginBottom: 16 }}><span className="muted">Cancel reason</span><br />{live.cancelReason}</p>}
            {showCancel && live.canRequestCancel && (
              <form onSubmit={submitCancelRequest} style={{ marginBottom: 16 }}>
            ...
            {!editing && (
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
                {live.canRequestCancel && !showCancel && <button type="button" className="btn" style={{ background: 'transparent', color: 'var(--ash)', border: '1px solid var(--iron)' }} onClick={() => setShowCancel(true)}>Request cancel</button>}
