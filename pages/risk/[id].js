...
import {
  ...
  applySourceLines, compute, emptyAssessment,
} from '../../lib/riskMath';
...
export default function RiskDetail() {
  ...
  function load() {
    ...
    authedFetch(`/api/risk/${id}`)
      .then(async (r) => {
        ...
        setForm({ ...blank, ...d.assessment, rs: { ...blank.rs, ...(d.assessment.rs || {}) } });
        setLoading(false);
      })
  ...
  return (
    <Layout active="risk" onNavigate={navigate}>
      ...
      {pay && (
        <div className="card">
          <h3 style={{ marginTop: 0 }}>{form.businessModel === 'Mall Paid Space' ? 'Mall paid space \u2014 session pricing' : 'Revenue sharing'}</h3>
          <div className="inline-form">
          ...
          {form.businessModel === 'Revenue Sharing' && (
            <>
              ...
              <p className="muted">Default 70 / 20 / 10 is pre-filled and editable. Mall paid space does not use this split.</p>
            </>
          )}
          {form.businessModel === 'Mall Paid Space' && (
            <>
              <Field label="Optional mall revenue share % of gross">
                <input type="number" disabled={locked} value={form.mallRevSharePct} onChange={(e) => set('mallRevSharePct', e.target.value)} />
              </Field>
              <p className="muted">LEMO keeps 100% of session gross less payment processing, monthly operating costs, and any optional mall revenue-share percentage. The RS 70/20/10 structure is not applied.</p>
            </>
          )}
          <div className="inline-form">
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                <tr><td>\u2212 Revenue distributions</td><td>{fmt((c.scenarios.low.venue || 0) + (c.scenarios.low.bd || 0) + (c.scenarios.low.mallShare || 0))}</td><td>{fmt((c.scenarios.base.venue || 0) + (c.scenarios.base.bd || 0) + (c.scenarios.base.mallShare || 0))}</td><td>{fmt((c.scenarios.high.venue || 0) + (c.scenarios.high.bd || 0) + (c.scenarios.high.mallShare || 0))}</td></tr>
